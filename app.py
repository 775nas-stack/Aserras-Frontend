...
def create_app(settings: Settings) -> FastAPI:
    ...
    async def api_chat(payload: ChatRequest):
        ...
        if payload.token:
            request_body["token"] = payload.token

        try:
            async with httpx.AsyncClient(timeout=20.0) as client:
                response = await client.post(
                    settings.brain_api_chat_send,
                    json=request_body,
            ...
            raise HTTPException(
                status_code=503,
                detail="The Aserras Brain is temporarily unreachable. Please try again shortly.",
            ) from exc

        if response.status_code >= 500:
            raise HTTPException(
                status_code=503,
                detail="The Aserras Brain is temporarily unavailable. Please try again soon.",
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=502,
                detail="Received an unexpected response from the Aserras Brain.",
            ) from exc
        ...
        if isinstance(reply, list):
            reply = "\n\n".join(str(part) for part in reply if part)

        if not isinstance(reply, str) or not reply.strip():
            reply = "We're ready when you are. The Brain didn't include a reply this time."

        return {"status": "ok", "reply": reply.strip(), "raw": data}
